...
public class ReadDataStudent{
    ...
    private double[][] data = new double[20000][2];
    ...
    public double[] getColumn(int col){
        double[] column = new double[20000];
        for (int row = 0; row < data.length; row++){
            column[row]=data[row][col+1];
        }
        return column;
    }
    ...
    //the correlation is the sum of the products of the standard units of the two columns divided by the number of values - 1
    ...
    public double correlation(double[] x, double[] y){
        // double sum = 0;
        // double[] standUx = standardUnits(x);
        // double[] standUy = standardUnits(y);
        // double productx = 1;
        // double producty = 1;
        // for (double d: standUx){
        //     productx *= d;
        // }
        // for (double d: standUy){
        //     producty *= d;
        // }
        // sum = productx+ producty;
        // return sum/(x.length-1);    
        double sum = 0;
        int i =0; for (double d: x) {
            sum+= d*y[i];
            i++;
        }
        return sum/21908;
